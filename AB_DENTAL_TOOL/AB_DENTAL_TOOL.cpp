...
void CreateGreyImage(vtkImageData* image, int Xn, int Yn, unsigned char* color) {
...
void ButtonCallbackFunction(vtkObject* caller, long unsigned int, void*, void*) {
	std::cout << "OK" << std::endl;
}


void visualize_mesh(Mesh movableMesh, std::string ID, Mesh staticMesh, double& Xoffset, double& Yoffset,
	...
	//vtkNew<vtkPolyData> movableVTKMesh = mesh_to_vtk(movableMesh, DEBUG);
	//vtkNew<vtkPolyDataConnectivityFilter> movableConnectivityFilter;
	//movableConnectivityFilter->SetInputData(movableVTKMesh);
	//movableConnectivityFilter->SetExtractionModeToAllRegions();
	//movableConnectivityFilter->ColorRegionsOn();
	//movableConnectivityFilter->Update();
	//vtkNew<vtkPolyDataMapper> movableMapper;
	//movableMapper->SetInputConnection(movableConnectivityFilter->GetOutputPort());
	//movableMapper->SetScalarModeToUseCellData();
	//movableMapper->Update();
	//movableActor->SetMapper(movableMapper);
	//MainRenderer->AddActor(movableActor);
	//InsetRenderer->AddActor(movableActor);
	//vtkNew<vtkUnsignedCharArray> movableCellColors;
	//movableCellColors->SetNumberOfComponents(3);
	//movableCellColors->SetName("Colors");
	//int numRegions = movableConnectivityFilter->GetNumberOfExtractedRegions();
	//vtkMath::RandomSeed(8775070); // Seed for consistent color generation
	//
	//for (int i = 0; i < numRegions; ++i) {
	//	unsigned char color[3];
	//	color[0] = static_cast<unsigned char>(vtkMath::Random(64, 255));
	//	color[1] = static_cast<unsigned char>(vtkMath::Random(64, 255));
	//	color[2] = static_cast<unsigned char>(vtkMath::Random(64, 255));
	//
	//	vtkNew<vtkPolyDataConnectivityFilter> movableShellFilter;
	//	movableShellFilter->SetInputData(movableVTKMesh);
	//	movableShellFilter->SetExtractionModeToSpecifiedRegions();
	//	movableShellFilter->InitializeSpecifiedRegionList();
	//	movableShellFilter->AddSpecifiedRegion(i);
	//	movableShellFilter->Update();
	//
	//	vtkPolyData* shell = movableShellFilter->GetOutput();
	//	for (vtkIdType j = 0; j < shell->GetNumberOfCells(); ++j) {
	//		movableCellColors->InsertNextTypedTuple(color);
	//	}
	//}
	////std::cout << numRegions << std::endl;
	//movableVTKMesh->GetCellData()->SetScalars(movableCellColors);
	...
	TextActor->GetTextProperty()->SetColor(1.0, 1.0, 1.0); // 0.7, 0.5, 0.3
	TextActor->SetDisplayPosition(10, 35);
	TextActor->SetInput(ID.c_str());
	...
	CreateGreyImage(Texture, width, height, TextureColor); // Use width and height variables
	...
	vtkNew<vtkCoordinate> BtnLocation;
	BtnLocation->SetCoordinateSystemToNormalizedDisplay();
	BtnLocation->SetValue(0.09, 0.15);
	double x = BtnLocation->GetComputedDisplayValue(MainRenderer)[0],
		y = BtnLocation->GetComputedDisplayValue(MainRenderer)[1];
	double bds[6] = {
		x - width, x + width,  // Adjust width boundaries
		y - height, y + height,  // Adjust height boundaries
		0.0, 0.0
	};
	...
	buttonWidget->AddObserver(vtkCommand::StateChangedEvent, buttoncallback);
...
void cut_mesh(Mesh& mesh, double height, bool DEBUG) {
...
bool modify_model_mesh(Mesh M_Mesh, Mesh F_Mesh, std::string O_Path, std::string ID_,
	...
	if (C_height > 0.0001)
		extrude_bottom_faces(M_Mesh, C_height, DEBUG);
	else if (C_height < -0.0001)
...
void process_files(const std::vector<std::string>& filenames, const std::string& filter, const std::string& inputPath,
	const std::string& outputPath, const Mesh& Fixture_Mesh, bool DEBUG) {
	std::cout << std::endl;
	...
	for (const auto& file : filtered_files) {
		...
		std::string Model_In_Path = inputPath + file + ".stl";
		std::string Model_Out_Path = outputPath + file + ".stl";

		if (!read_STL(Model_In_Path, Model_Mesh, DEBUG)) continue;
		Visual_mesh = Model_Mesh;

		if (file == filtered_files[0]) {
			Point centroid;
			get_centroid(Visual_mesh, centroid, DEBUG);
			translate_mesh(Visual_mesh, Kernel::Vector_3(-centroid.x(), -centroid.y(), 0), DEBUG);
			visualize_mesh(Visual_mesh, file, Fixture_Mesh, Model_Xoffset, Model_Yoffset, cut_height, Model_Zrot, true);
		}
		std::cout << "    > X" << Model_Xoffset << "  Y" << Model_Yoffset << "  CH" << cut_height << "  R" << Model_Zrot << std::endl;
		modify_model_mesh(Model_Mesh, Fixture_Mesh, Model_Out_Path, file, Model_Xoffset, Model_Yoffset, cut_height, Model_Zrot, DEBUG);
	}
}
...
int main(int argc, char* argv[]) {
	...
	for (const auto& pair : groupedFiles) sortedKeys.push_back(pair.first);
	std::sort(sortedKeys.begin(), sortedKeys.end());

	for (const auto& key : sortedKeys) {
		std::cout << Green << "      Processing CaseID: " << ColorEnd << key << std::endl;
		for (const auto& filename : groupedFiles[key]) std::cout << "      > " << Cyan << filename << ColorEnd << std::endl;

		// LOWER LOWER LOWER LOWER LOWER LOWER LOWER LOWER LOWER LOWER LOWER LOWER LOWER LOWER LOWER LOWER LOWER
		process_files(groupedFiles[key], "L", inputPath, outputPath, Fixture_Mesh, DEBUG);
		// UPPER UPPER UPPER UPPER UPPER UPPER UPPER UPPER UPPER UPPER UPPER UPPER UPPER UPPER UPPER UPPER UPPER 
		process_files(groupedFiles[key], "U", inputPath, outputPath, Fixture_Mesh, DEBUG);
	}
...
//// LOWER LOWER LOWER LOWER LOWER LOWER LOWER LOWER LOWER LOWER LOWER LOWER LOWER LOWER LOWER LOWER LOWER
		...
		//std::copy_if(groupedFiles[key].begin(), groupedFiles[key].end(), std::back_inserter(filtered_Lower),
		//	[](const std::string& name) {
		//		return name.find("L") != std::string::npos;
		//	});
		//std::sort(filtered_Lower.begin(), filtered_Lower.end());
		//
		//Model_Xoffset = 0.0, Model_Yoffset = 0.0, cut_height = 0.0, Model_Zrot = 0.0;
		...
		//	Visual_mesh = Model_Mesh;
		//
		//	std::cout << Model_Xoffset << "   " << Model_Yoffset << "   " << cut_height << "   " << Model_Zrot << std::endl;
		//
		//	if (LOWER == filtered_Lower[0]) {
		//		Point centroid;
		//		get_centroid(Visual_mesh, centroid, DEBUG);
		//		translate_mesh(Visual_mesh, Kernel::Vector_3(-centroid.x(), -centroid.y(), 0), DEBUG);
		//		visualize_mesh(Visual_mesh, LOWER, Fixture_Mesh, Model_Xoffset, Model_Yoffset, cut_height, Model_Zrot, true);
		//	}
		//
		...
		//std::copy_if(groupedFiles[key].begin(), groupedFiles[key].end(), std::back_inserter(filtered_Upper),
		//	[](const std::string& name) {
		//		return name.find("U") != std::string::npos;
		//	});
		//std::sort(filtered_Upper.begin(), filtered_Upper.end());
		//
		//Model_Xoffset = Model_Yoffset = cut_height = Model_Zrot = 0.0;
		...
		//	std::cout << Model_Xoffset << "   " << Model_Yoffset << "   " << cut_height << "   " << Model_Zrot << std::endl;
		//
		//	if (UPPER == filtered_Upper[0]) {
		//		Point centroid;
		//		get_centroid(Visual_mesh, centroid, DEBUG);
		//		translate_mesh(Visual_mesh, Kernel::Vector_3(-centroid.x(), -centroid.y(), 0), DEBUG);
		//		visualize_mesh(Visual_mesh, UPPER, Fixture_Mesh, Model_Xoffset, Model_Yoffset, cut_height, Model_Zrot, true);
		//	}
		//
